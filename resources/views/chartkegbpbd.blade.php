...
<head>
    <?php 
$bencana = DB::table('kegiatans')->get();
?>
  <meta charset="utf-8">
...
<body id="page-top">
  ...
  <div id="wrapper">
    ...
    <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
      ...
      <!-- Nav Item - Tables -->
      <li class="nav-item">
        <a class="nav-link" href="{{ route('chart2bpbd') }}">
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-2 text-gray-800">Grafik dibawah adalah laporan data pemetaan lokasi yang ada di wilayah sleman</h1>
...
var ctx = document.getElementById("myChart").getContext('2d');
    var myChart = new Chart(ctx, {
      type: 'bar',
      data: {
        labels: ["Sosialisasi", "Pelatihan", "Seminar"],
        datasets: [{
          label: '',
          data: [
          <?php 
          $jumlah_sosialisasi = DB::table('kegiatans')
                        ->select('kegiatans.*')
                        ->where('jenisKeg','Sosialisasi')
                        ->get();
                $num_rows = count($jumlah_sosialisasi);


          echo "$num_rows";
          ?>, 
          <?php 
          $jumlah_pelatihan =DB::table('kegiatans')
                        ->select('kegiatans.*')
                        ->where('jenisKeg','Pelatihan')
                        ->get();
          echo count($jumlah_pelatihan);
          ?>,
          <?php 
          $jumlah_seminar =DB::table('kegiatans')
          ->select('kegiatans.*')
          ->where('jenisKeg','Seminar')
          ->get();
echo count($jumlah_seminar);
          ?>,
