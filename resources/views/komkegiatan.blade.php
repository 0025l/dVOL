...
?>
    ...
    <section class="portfolio" id="portfolio">
      <div class="container">
        <h2 class="text-center text-uppercase text-secondary mb-0">Pilih Komunitas</h2>
...
<form action="{{ route('komkeg1.store') }}" method="POST">
        @foreach($data as $datas)
                {{ csrf_field() }}
                <div class="form-group">
                  <div id="myHeader">
                    <label for="id_benc">ID Kegiatan:</label>
                    <input name="id_keg" readonly="true" value="{{ $datas->id_keg }}">
                <br />
                ...
                <div class="form-group">
                  <div id="myHeader2">
                  <label for="relawan">Pilih Komunitas yang akan hadir : </label><br>
                    @foreach($komunitas as $dataya)
                    <input type="checkbox" name="komunitas[]" value="{{ $dataya->nama_kom }}">{{ $dataya->nama_kom}}<br>
                @endforeach
</div>
</div>
                <div class="form-group">
